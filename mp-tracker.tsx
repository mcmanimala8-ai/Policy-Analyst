...
const PARTY_STYLES: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  "Dravida Munnetra Kazhagam": { bg: "bg-red-50", text: "text-red-800", border: "border-red-200", dot: "bg-red-600" },
  "Indian National Congress": { bg: "bg-blue-50", text: "text-blue-800", border: "border-blue-200", dot: "bg-blue-600" },
  "Viduthalai Chiruthaigal Katchi": { bg: "bg-indigo-50", text: "text-indigo-800", border: "border-indigo-200", dot: "bg-indigo-600" },
  "Communist Party of India (Marxist)": { bg: "bg-rose-50", text: "text-rose-900", border: "border-rose-200", dot: "bg-rose-700" },
  "Communist Party of India": { bg: "bg-rose-50", text: "text-rose-900", border: "border-rose-200", dot: "bg-rose-700" },
  "Indian Union Muslim League": { bg: "bg-green-50", text: "text-green-800", border: "border-green-200", dot: "bg-green-600" },
  "Marumalarchi Dravida Munnetra Kazhagam": { bg: "bg-orange-50", text: "text-orange-800", border: "border-orange-200", dot: "bg-orange-600" },
};
const DEFAULT_PARTY_STYLE = { bg: "bg-slate-100", text: "text-slate-700", border: "border-slate-200", dot: "bg-slate-500" };
function partyStyle(party: string) { return PARTY_STYLES[party] ?? DEFAULT_PARTY_STYLE; }

const STATE_AVG = { attendance: 79.2, questions_asked: 113, debates_participated: 24 };
const MP_BRIEFS: Record<string, string> = { "Thoothukkudi": "/tracker/kanimozhi-thoothukkudi" };
...
function MetricBar({ label, value, max, suffix = "", barClass = "bg-slate-700", note }: {
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="font-semibold text-slate-800 tabular-nums">{value}{suffix}</span>
      </div>
      ...
      {value
        ...
        : (
          <>
            ...
            <p className="text-xs text-slate-400 italic mt-1">📊 Qualitative briefs in preparation — quantitative data live.</p>
          </>
        )
      }
    </div>
...
export function MPTracker() {
  ...
  if (!data.length) {
    return <section className="py-20 text-center text-slate-400 text-sm">Loading legislative data…</section>;
  }
    ...
    <section id="data" className="bg-white py-20 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-12">
          ...
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight mb-2">Tamil Nadu MP Performance Tracker</h2>
          <p className="text-slate-500 text-sm max-w-xl">Live legislative metrics for all 39 Lok Sabha constituencies — 18th Lok Sabha, sourced from PRS India.</p>
        </div>

        <div className="relative mb-10 max-w-md">
          <div className="flex items-center gap-3 px-4 py-3 bg-white border border-slate-200 rounded-xl shadow-sm cursor-pointer hover:border-slate-400 transition-colors" onClick={() => setOpen((o) => !o)}>
            <MapPin className="w-4 h-4 text-slate-400 shrink-0" />
            <span className="flex-1 text-sm font-medium text-slate-700 truncate">{selected ? `${selected.constituency} — ${selected.mp_name}` : "Select constituency…"}</span>
          ...
          {open && (
            <div className="absolute z-20 top-full mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden">
              <div className="p-2 border-b border-slate-100">
                <input autoFocus className="w-full px-3 py-2 text-sm bg-slate-50 rounded-lg outline-none placeholder:text-slate-400 text-slate-800" placeholder="Search constituency or MP name…" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
              <ul className="max-h-64 overflow-y-auto divide-y divide-slate-50">
                {filtered.map((mp) => (
                  <li key={mp.constituency} onClick={() => { setSelected(mp); setOpen(false); setSearch(""); }} className={`px-4 py-2.5 cursor-pointer hover:bg-slate-50 transition-colors flex items-center justify-between ${selected?.constituency === mp.constituency ? "bg-slate-50" : ""}`}>
                    <div>
                      <p className="text-sm font-medium text-slate-800">{mp.constituency}</p>
                      <p className="text-xs text-slate-400">{mp.mp_name}</p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${partyStyle(mp.party).bg} ${partyStyle(mp.party).text}`}>{mp.party.split(" ").map(w => w[0]).join("").slice(0, 4)}</span>
                  </li>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-16">
            <div className="lg:col-span-1 rounded-2xl border border-slate-100 bg-gradient-to-br from-slate-900 to-slate-800 p-6 text-white flex flex-col justify-between shadow-md">
              ...
              {MP_BRIEFS[selected.constituency] && (
                <a href={MP_BRIEFS[selected.constituency]} className="mt-4 flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-xl bg-indigo-500/20 border border-indigo-400/30 text-indigo-300 text-xs font-semibold tracking-wide hover:bg-indigo-500/30 transition-colors">
            ...
            <div className="lg:col-span-2 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm space-y-6">
              ...
              <MetricBar label="Questions Asked" value={selected.questions_asked} max={300} barClass="bg-slate-700" note={`State avg: ${STATE_AVG.questions_asked}`} />
              <MetricBar label="Debates Participated" value={selected.debates_participated} max={200} barClass="bg-indigo-500" note={`State avg: ${STATE_AVG.debates_participated}`} />
              <MetricBar label="Private Member Bills" value={selected.private_member_bills} max={10} barClass="bg-amber-500" />
            </div>
        ...
        <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="text-sm font-semibold tracking-widest uppercase text-slate-500">Compare MPs Side by Side</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            {[{ val: cmpA, set: setCmpA, label: "MP One" }, { val: cmpB, set: setCmpB, label: "MP Two" }].map(({ val, set, label }) => (
              <div key={label}>
                ...
                <select value={val} onChange={(e) => set(e.target.value)} className="w-full px-3 py-2.5 text-sm bg-white border border-slate-200 rounded-xl text-slate-700 outline-none focus:border-slate-400 transition-colors appearance-none cursor-pointer">
          ...
          {mpA && mpB && (
            <>
              <div className="flex flex-wrap gap-3 mb-6">
                {[{ mp: mpA, color: "#1e293b" }, { mp: mpB, color: "#6366f1" }].map(({ mp, color }) => (
                  <div key={mp.constituency} className="flex items-center gap-2">
                    <span className="w-3 h-3 rounded-sm inline-block" style={{ backgroundColor: color }} />
                    <span className="text-xs text-slate-600 font-medium">{mp.mp_name}</span>
        ...
        <p className="mt-6 text-xs text-slate-400 text-right">Source: PRS India · 18th Lok Sabha · Data as of June 2026</p>
      </div>
    </section>
  );
}
